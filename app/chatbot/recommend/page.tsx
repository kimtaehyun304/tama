...
export default function ChatPage() {
  const [messages, setMessages] = useState<ReommendChatMessage[]>([]);
  ...
  const handleSend = async () => {
    ...
    const userMessage: ReommendChatMessage = {
      role: "user",
      content: userInput,
    };
    ...
    try {
      ...
      */
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/chatbot/recommend`,
        {
          method: "post",
          headers: {
      ...
      const assistantRes: ReommendChatMessage = await response.json();
